...
int main()
{
    ...
    int sockfd = socket(IPV4, SOCK_DGRAM, IPPROTO_UDP);
    ...
    server_addr.sin_port = htons(SERVER_PORT); //=>16 сетевого пор.

    int inet_pton_ret = inet_pton(IPV4, "127.0.0.1", &server_addr.sin_addr); //=>2
    if (inet_pton_ret <= 0)
    {
